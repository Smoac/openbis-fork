 ...
 */
public class PGPLiteralDataGenerator implements Closeable
{
    ...
    /**
     * Generates literal data objects in the old format, this is important if you need compatability
     * with PGP 2.6.x.
     * 
     * @param oldFormat
     */
    ...
    private void writeHeader(OutputStream out, char format, byte[] encodedName, long modificationTime)
            throws IOException
    {
        out.write(format);
    ...
    static class WrappedGeneratorStream extends OutputStream
    {
     ...
     */
    public OutputStream open(OutputStream out, char format, String name, long length,
            Date modificationTime) throws IOException
    {
        if (pkOut != null)
        {
        ...
        byte[] encodedName = Strings.toUTF8ByteArray(name);
        
        pkOut =
                new BCPGOutputStream(out, PacketTags.LITERAL_DATA, length + 2 + encodedName.length + 4,
                        oldFormat);

        writeHeader(pkOut, format, encodedName, modificationTime.getTime());
     ...
     */
    public OutputStream open(OutputStream out, char format, String name, Date modificationTime,
    ...
    public OutputStream open(OutputStream out, char format, File file) throws IOException
    {
        ...
        byte[] encodedName = Strings.toUTF8ByteArray(file.getName());
        
        pkOut =
                new BCPGOutputStream(out, PacketTags.LITERAL_DATA, file.length() + 2
                        + encodedName.length + 4, oldFormat);

        writeHeader(pkOut, format, encodedName, file.lastModified());

        return new WrappedGeneratorStream(pkOut, this);
    }

    /**
     * Close the literal data packet - this is equivalent to calling close on the stream returned by
     * the open() method.
     * 
     * @throws IOException
     */
