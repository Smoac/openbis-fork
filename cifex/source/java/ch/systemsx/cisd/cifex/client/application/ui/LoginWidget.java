...
/**
 * A <code>Form</code> extension that is the login panel we use to login to the
 * <i>openBIS</i> system.
 * 
...
public class LoginWidget extends VerticalPanel {
	...
	private final Button createButton() {
		final Button b = new Button(context.getMessageResources()
				.getLoginButtonLabel());
	...
	private final TextField<String> createPasswordField() {
		...
		field.setFieldLabel(context.getMessageResources()
	...
	private final void doLogin(final ViewContext viewContext) {
		if (formPanel.isValid()) {
			...
			viewContext.getCifexService().tryLogin(user, password,
					new LoginAsyncCallBack());
		}
	}
	...
	private final class LoginAsyncCallBack extends
		...
		public final void onSuccess(final UserInfoDTO result) {
			if (result != null) {
				...
				final IMessageResources messageResources = context
						.getMessageResources();
				final String title = messageResources
						.getMessageBoxWarningTitle();
				MessageBox.alert(title, messageResources
						.getLoginFailedMessage(), null);
				getButton().enable();
			}
		}
	}
