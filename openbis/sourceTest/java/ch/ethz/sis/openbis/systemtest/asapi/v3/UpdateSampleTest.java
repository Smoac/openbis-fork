...
public class UpdateSampleTest extends AbstractSampleTest
{
    ...
    public void testUpdateWithDifferentRolesProjectSample(RoleWithHierarchy role)
    {
        testWithUserRole(role, params ->
        {
            ...
            if (Arrays.asList(RoleWithHierarchy.RoleCode.ADMIN, RoleWithHierarchy.RoleCode.POWER_USER).contains(role.getRoleCode()))
            {
                v3api.updateSamples(params.userSessionToken, Collections.singletonList(sampleUpdate));
            } else
            {
    ...
    public void testUpdateWithDifferentRolesExperimentSample(RoleWithHierarchy role)
    {
        testWithUserRole(role, params ->
        {
            ...
            final List<ExperimentPermId> experimentIds =
                    v3api.createExperiments(params.adminSessionToken, Arrays.asList(experimentCreation, experimentCreation2));
    ...
    public void testUpdateWithDifferentRolesParentChildSample(RoleWithHierarchy role)
    {
        testWithUserRole(role, params ->
        {
            ...
            parentCreation.setChildIds(Collections.singletonList(childCreation.getCreationId()));
            childCreation.setParentIds(Collections.singletonList(parentCreation.getCreationId()));

            List<SamplePermId> sampleIds = v3api.createSamples(params.adminSessionToken, Arrays.asList(parentCreation, childCreation));
            ...
            {
                v3api.updateSamples(params.userSessionToken, Arrays.asList(parentUpdate, childUpdate));
            } else
            {
                assertAnyAuthorizationException(() -> v3api.updateSamples(params.userSessionToken, Arrays.asList(parentUpdate, childUpdate)));
            }
        });
    }
