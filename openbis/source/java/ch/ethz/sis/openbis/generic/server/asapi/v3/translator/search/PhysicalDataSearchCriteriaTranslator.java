 ...
 * @author pkupczyk
 */
public class PhysicalDataSearchCriteriaTranslator extends AbstractCompositeSearchCriteriaTranslator
{

    public PhysicalDataSearchCriteriaTranslator(IDAOFactory daoFactory, IObjectAttributeProviderFactory entityAttributeProviderFactory)
    {
    ...
    {
        return criteria instanceof PhysicalDataSearchCriteria;
    }
    ...
    protected SearchCriteriaTranslationResult doTranslate(SearchTranslationContext context, ISearchCriteria criteria)
    {
        ...
        {
            throw new IllegalArgumentException("Physical data criteria can be used only in data set criteria, "
                    + "but was used in: " + context.peekObjectKind() + " context.");
        }

        context.pushObjectKind(SearchObjectKind.PHYSICAL_DATA);
