...
import ch.ethz.sis.openbis.generic.server.asapi.v3.helper.plugin.ListPluginByPermId;
 ...
 */
@Component
public class MapPluginByIdExecutor extends AbstractMapObjectByIdExecutor<IPluginId, ScriptPE>
        implements IMapPluginByIdExecutor
{
    private IScriptDAO scriptDAO;

    @Override
    protected void addListers(IOperationContext context, List<IListObjectById<? extends IPluginId, ScriptPE>> listers)
    {
        listers.add(new ListPluginByPermId(scriptDAO));
    }

    @Autowired
    private void setDAOFactory(IDAOFactory daoFactory)
    {
        scriptDAO = daoFactory.getScriptDAO();
    }
