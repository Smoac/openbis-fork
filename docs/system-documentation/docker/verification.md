...
```
$ docker exec -it openbis-server /home/openbis/servers/openBIS-server/jetty/bin/status.sh;
openBIS Application Server is running (pid 24538)
```

Check version of running openBIS Application Server.
```
$ docker exec -it openbis-server /home/openbis/servers/openBIS-server/jetty/bin/version.sh;
20.10.7 (r1688387419)
```

Check the password file for file based authentication.
```
$ docker exec -it openbis-server /home/openbis/servers/openBIS-server/jetty/bin/passwd.sh list;
...
```
$ docker exec -it openbis-server curl http://localhost:8080/openbis/webapp/eln-lims/version.txt;
20.10.7
```

Examine a process of  openBIS Data Store Server.
```
$ docker exec -it openbis-server pgrep -af DataStoreServer;
...
```
$ docker exec -it openbis-server psql -h postgres15 -U openbis openbis_prod -c "select id,user_id,email from persons";
